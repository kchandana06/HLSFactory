...
def run_cpp_preprocessor(design_dir: Path, define_statement: dict[str, Any]) -> None:
    """Run the C++ preprocessor over every C/C++ source file under ``design_dir``.

    Each matched source file is replaced in place with its preprocessed
    output. ``-D`` flags are built from ``define_statement``: ``None`` or
    ``True`` emits a bare ``-DKEY``, ``False`` omits the flag entirely, and
    any other value emits ``-DKEY=value``.
    """
...
class CPPPreprocessorFrontend(Frontend):
    ...
    def execute(self, design: Design, timeout: float | None = None) -> list[Design]:
        ...
        for define_statement in self.define_statements:
            ...
            run_cpp_preprocessor(new_design.dir, define_statement)
